...
@export
@wr.vectorize_first
def rate_elasticdf(erec, mw, sigma_nucleon, interaction='SI',
                 m_med=float('inf'), t=None, material='Xe',
    ...
    v_min = vmin_elastic(erec, mw, material)*nu.s/nu.km
    v_max=wr.v_max(t, v_esc*nu.km/nu.s,v_0=v_0*nu.km/nu.s)*nu.s/nu.km
    if(v_max<v_min):
        return 0
    fac=nu.Msolar/(nu.kpc)**3/(mw*mn(material))
    try:
        len(erec)
    except TypeError:
        def S0(v):
            if(v<v_min or v>v_max):
                return 0
            return fac*sigma_erec(erec, v*nu.km/nu.s, mw, sigma_nucleon,
        ...
        for j in range(len(erec)):
            def S0(v):
                if(v<v_min or v>v_max):
                    return 0
                return fac*sigma_erec(erec[j], v*nu.km/nu.s, mw, sigma_nucleon,
                                interaction, m_med, material=material)*v*nu.km/nu.s
            def sf(xv):
                v=np.array((((xv[:,3]-ve[0])**2+(xv[:,4]-ve[1])**2+(xv[:,5]-ve[2])**2))**0.5)
                try:
