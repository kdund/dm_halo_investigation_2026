...
def f(phi,csth,v,vel_earth_t,df,e1,e2,e3):
    ...
    jac=v**2
    conv=nu.km/nu.s
    return jac*df(np.array([vR,vp,vz])/conv)/conv**3
@export
def observed_speed_dist2(v,t=None,distF=None,v_0=None,vesc=None):
    v_0 = _HALO_DEFAULTS['v_0'] * nu.km/nu.s if v_0 is None else v_0
    if distF is None:
        v_0n = _HALO_DEFAULTS['v_0']
        v_esc = _HALO_DEFAULTS['v_esc']
        distF=dfHalo(v_0n,v_esc)
    t1=t
    if t is None:
        t1=59.37
    vel_earth_t = earth_velocity(t1,v_0)
    vel_earth_t[0]*=-1
    vel_earth_t[1]*=-1
    ...
    try:
        ...
        csthmax=(vesc**2-vearth**2-v**2)/(2*v*vearth)
        if(csthmax<-1):
            return 0
        if(csthmax>1):
            csthmax=1
        return dblquad(f,-1,csthmax,0,2*np.pi,args=(v,vel_earth_t,distF,e1,e2,e3),epsrel=1e-1,epsabs=0)[0]
    else:
        result=np.zeros(len(v))
        for i in range(len(v)):
            csthmax=(vesc**2-vearth**2-v[i]**2)/(2*v[i]*vearth)
            if(csthmax<-1):
                result[i]=0
                continue
            if(csthmax>1):
                csthmax=1
            result[i]=dblquad(f,-1,csthmax,0,2*np.pi,args=(v[i],vel_earth_t,distF,e1,e2,e3),epsrel=1e-1,epsabs=0)[0]
        return result
